...
switch ($act) {
	case 'delete':
		$res_1 = $db->delete('analogies', "`item_id`=".$_GET['item_id']." AND `item_anal`=".$_GET['delete_item']);
		$res_2 = $db->delete('analogies', "`item_id`=".$_GET['delete_item']." AND `item_anal`=".$_GET['item_id']);
		if ($res_1 and $res_2){
	...
	case 'add_item':
		$res_1 = $db->insert('analogies', array('item_id' => $_GET['item_id'], 'item_anal' => $_GET['id']));
		$res_2 = $db->insert('analogies', array('item_id' => $_GET['id'], 'item_anal' => $_GET['item_id']));
		if ($res_1 and $res_2){
...
function item(){
	...
	$status = "<a href='/admin'>Главная</a> > <a href='?view=analogies'>Аналоги</a> > $page_title";
	$analogies = $db->select('analogies', 'item_anal', "`item_id`=$item_id");?>
	...
	<table style="" class="t_table" cellspacing="1">
		...
		<?if (count($analogies)){
			foreach($analogies as $analogy){
				$item = $db->select('items', 'id,article,title_full,barcode,brend_id', "`id`=".$analogy['item_anal']);
				$item = $item[0]?>
				<tr>
					<td><?=$db->getFieldOnID('brends', $item['brend_id'], 'title')?></td>
					<td><a href="?view=item&id=<?=$item['id']?>"><?=$item['article']?></a></td>
...
function item_search(){
	...
	if (!$search) header ('Location: ?view=analogies');
	$is_subtitutes = $db->select('analogies', "item_anal", "`item_id`=$item_id");
	if (count($is_subtitutes)){
		$in = "";
		foreach ($is_subtitutes as $value) $in .= $value['item_anal'].',';
		$in = substr($in, 0, -1);
	} 
