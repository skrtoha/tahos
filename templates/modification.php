...
if (isset($_GET['carid'])){
	$nodesPartsCatalogs = PartsCatalogs::getNodes($_GET['brend'], $_GET['carid']);
	$modelInfoPartsCatalogs = PartsCatalogs::getModelInfoByModelID($_GET['brend'], $_GET['href']);
	// debug($nodesPartsCatalogs);
}
else{
	$query = "
		SELECT
	...
	<div class="items">
		...
		<div class="content" brend="<?=$model['brend']?>">
			...
			<div class="mosaic-view clearfix">
				...
				<?if(isset($nodesPartsCatalogs)){?>
					<?foreach($nodesPartsCatalogs as $node){?>
						<div class="item">
