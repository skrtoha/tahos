...
switch ($act) {
	...
	case 'delete_related':
		...
		header("Location: /admin/?view=item&act=related&id={$_GET['item_id']}");
		break;
	case 'related_add':
	...
	case 'delete_complect':
		...
		message('Успешно удалено!');
		header("Location: /admin/?view=item&act=complect&id={$_GET['item_id']}");
		break;
	case 'complect_add':
	...
	case 'analogies_add': analogies_substitutes_add('analogies'); break;
...
function show_form($act){
	...
	<? if ($act != 's_add'){?> 
		<div id="need_similar">
			...
			$class = $count ? 'red' : '';?>
			<a class="<?=$class?>" href="?view=item&act=complect&id=<?=$item['id']?>">Комплектность(<?=$count?>)</a>
			<?$count = $db->getCount('articles', "`item_id`={$item['id']} AND `item_diff`<>{$item['id']}");
			$class = $count ? 'red' : '';?>
			<a class="<?=$class?>" href="?view=item&act=related&id=<?=$item['id']?>">Подобные(<?=$count?>)</a>
			<?$count = $db->getCount('store_items', "`item_id`=".$item['id']);
			$class = $count ? 'red' : '';?>
			<a class="<?=$class?>" href="?view=item&act=prices&id=<?=$item['id']?>">Прайсы(<?=$count?>)</a>
			<?$count = $db->getCount('substitutes', "`item_id`=".$item['id']);
			$class = $count ? 'red' : '';?>
			<a class="<?=$class?>" href="?view=item&act=substitutes&item_id=<?=$item['id']?>">Замены(<?=$count?>)</a>
			<?$count = $db->getCount('analogies', "`item_id`=".$item['id']);
			$class = $count ? 'red' : '';?>
			<a class="<?=$class?>" href="?view=item&act=analogies&item_id=<?=$item['id']?>">Аналоги(<?=$count?>)</a>
			<a href="?view=item&act=history&item_id=<?=$item['id']?>">История</a>
					...
					<div class="field">
						...
						<div class="value" id="properties">
							<?$res_items_values = $db->query("
								...
								FROM
									#items_values iv
								LEFT JOIN #filters_values fv ON fv.id=iv.value_id
								LEFT JOIN #filters f ON f.id=fv.filter_id
								WHERE
									iv.item_id={$item['id']}
							", '');
							if ($res_items_values->num_rows) while($row = $res_items_values->fetch_assoc()){
								$items_values[$row['filter_id']] = $row['value_title'];
							}
							$res_filters = $db->query("
								...
								WHERE
									f.category_id IN (
										SELECT category_id FROM #categories_items 
										WHERE item_id={$_GET['id']}
									)
								ORDER BY f.pos ASC
							", '');
							if ($res_filters->num_rows) while($r = $res_filters->fetch_assoc()){
								$filters[$r['category_id']]['title'] = $r['category_title'];
								$filters[$r['category_id']]['filters'][$r['filter_id']]['title'] = $r['filter_title'];
								$filters[$r['category_id']]['filters'][$r['filter_id']]['filter_values'][$r['value_id']] = $r['value_title'];
							}
							if (!empty($filters)){?>
								<div id="category_items" item_id="<?=$_GET['id']?>">
									<?foreach($filters as $category_id => $category){
										$c = & $category;?>
										<div class="category_item">
											<p><b><?=$c['title']?></b><a href="#" class="item_filter">Изменить</a></p>
											<?if (!empty($c['filters'])){?>
												<table class="category" category_id="<?=$category_id?>">
													<?foreach($c['filters'] as $filter_id => $filter){?>
														<tr>
															<td><?=$filter['title']?></td>
															<td filter_id="<?=$filter_id?>"><?=$items_values[$filter_id]?></td>
														</tr>
													<?}?>
												</table>
											<?}?>
	...
	<form style="display: none" id="upload_image" action="/admin/?view=item&id=<?=$item['id']?>?>" enctype="multipart/form-data" method="post">
...
function prices(){
	...
	$page_title = "Прайсы";
	$status = "<a href='/admin'>Главная</a> > <a href='?view=items'>Номенклатура</a> > ";
	$status .= "<a href='?view=item&id=$id&act=change'>".$db->getFieldOnID('items', $id, 'article')."</a> > $page_title";
...
function related(){
	...
	$status = "<a href='/admin'>Главная</a> > <a href='?view=items&id=$item_id'>{$item['title_full']}</a> > $page_title";?>
...
function related_search(){
	...
	if ($res_items->num_rows > 100){
		...
		header("Location: /admin/?view=item&act=related&id=$item_id");
	}
		...
		<?if ($res_items->num_rows){
			while($value = $res_items->fetch_assoc()){?>
				<tr>
					...
					<td><?=$value['barcode']?></td>
					<td><a href="?view=item&act=related_add&item_related=<?=$value['id']?>&item_id=<?=$item_id?>">Добавить</a></td>
				</tr>
			<?}	
		}
		else{?>
...
function item_search(){
	...
	$item = $item[0];
	$page_title = "Добавелние замены для товара";
...
function complect(){
	...
	<div id="total" style="margin-top: 20px">Всего: <?=count($items)?></div>
	<div style="margin-top: 5px" class="actions">
		<form action="/admin/?view=item&act=complect_search&id=<?=$item_id?>" method="post">
			<input style="width: 264px;" type="text" name="search" value="<?=$_POST['search']?>" placeholder="Поиск для добавления">
			<input type="submit" value="Искать">
		</form>
	</div>
...
function complect_search(){
	...
	if ($res_items->num_rows > 100){
		message('Слишком много совпадений! Уточните поиск.', false);
		header("Location: /admin/?view=item&act=complect&id=$item_id");
	}
	$page_title = "Поиск для добавления";
	$status = "<a href='/admin'>Главная</a> > $page_title";?>
	<a href="?view=item&id=<?=$item_id?>" style="margin-bottom: 10px;display: block">Карточка товара</a>
...
function analogies_substitutes($type){
	...
	$status = "
		...
		<a href='/admin/?view=item&id={$_GET['item_id']}'>{$item['brend']} - {$item['article']}</a> >
		$page_title
	";
	$res_items = $db->query("
		SELECT
	...
	<div class="actions">
		<form style="margin-top: -3px;float: left;margin-bottom: 10px;" action="?view=item&act=<?=$type?>_search&item_id=<?=$item_id?>" method="post">
			<input style="width: 264px;" type="text" name="search" value="<?=$_POST['search']?>" placeholder="Поиск для добавления">
			<input type="submit" value="Искать">
		</form>
	</div>
...
function analogies_substitutes_search($type){
	...
	$status = "
		...
		<a href='/admin/?view=items'>Номенклатура</a> >
		<a href='/admin/?view=item&id={$_GET['item_id']}'>{$item['brend']} - {$item['article']}</a> >
		<a href='/admin/?view=item&act=$type&item_id={$_GET['item_id']}'>$status_title</a> >
		$page_title
	";
	...
	<div class="actions" style="width: 100%">
		<form style="margin-top: -3px;float: left;margin-bottom: 10px;" action="?view=item&act=<?=$type?>_search&item_id=<?=$_GET['item_id']?>" method="post">
			<input style="width: 264px;" type="text" name="search" value="<?=$_POST['search']?>" placeholder="Поиск">
			<input type="submit" value="Искать">
		</form>
	</div>
		...
		<?if (count($items)){
			foreach($items as $id => $item){?>
				<tr>
					...
					<td><a class="<?=$type?>_add" href="?view=item&act=<?=$type?>_add&item_id=<?=$_GET['item_id']?>&added=<?=$id?>">Добавить</a></td>
				</tr>
			<?}	
		}
		else{?>
...
function analogies_substitutes_add($type){
	...
	message('Успешно добавлено!');
	header("Location: ?view=item&act=$type&item_id={$_GET['item_id']}");
}
